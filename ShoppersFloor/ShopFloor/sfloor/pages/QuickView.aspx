...
<%--<link href="http://localhost:6608/wwwroot_new/CSS/internalpage.css" rel="stylesheet" type="text/css" />--%>
...
<link href="http://localhost:14880/wwwroot_new/CSS/QuickView.css" rel="stylesheet" type="text/css" />
    <link href="http://web3.128.210.new.ocpwebserver.com/CSS/breadcrumb.css" rel="stylesheet" type="text/css" />
    
    <script src="http://web3.128.210.new.ocpwebserver.com/js/tooltip.js" type="text/javascript"></script>
    <script src="http://web3.128.210.new.ocpwebserver.com/js/formbox.js" type="text/javascript"></script>
    <link href="http://web3.128.210.new.ocpwebserver.com/CSS/sprite.css" rel="stylesheet" type="text/css" />
    
<script src="http://web3.128.210.new.ocpwebserver.com/js/jquery.contenthover.min.js" type="text/javascript"></script>
     <style type="text/css">
    ...
    <script type="text/javascript">
        function selectSize(txt) {
            $("input[namespace='sizeBtn']").css({ 'background-color': '#EBEBEB', 'color': 'black' })
            $("input[namespace='sizeBtn'][value='" + txt + "']").css({ 'background-color': '#464646', 'color': 'white' });
            $("#size").val(txt);
        }
        function addtoCart() {
            ...
            $.ajax({
                ...
                url: "<%=ConfigUtil.hostURL()%>QuickView.aspx?action=addToCart&sku=" + $('#style').val() + "&qty=" + $("#dd_quantity option:selected").val() + "&color=" + $("#color").val() + "&size=" + $("#size").val() + "&isColor=" + $("#isColor").val() + "&isSize=" + $("#isSize").val() + "&isSku=" + $("#isSku").val(),
        ...
        function quickView(sku) {
            ...
            $.ajax({
                type: "get",
                contentType: "application/json; charset=utf-8",
                url: "<%=ConfigUtil.hostURL()%>QuickView.aspx?sku=" + sku,

                beforeSend: function () {
                    $("#forgetLoader").css("display", "block");
                },
                success: function (data) {
                    document.getElementById('signuppopup2light').style.display = 'block';
                    $("#forgetLoader").css("display", "none");
                    $('#qViewContent').html(data);
                    //document.getElementById('signuppopup2fade').style.display = 'block';
...
<div class="contenttopsecondlayer">
    ...
    <div class="productcart">
        ...
        <%}else {%>
            <input type='button' id="btn_addtocart"  onclick='addtoCart();' value="Add To Cart" 
                class="btn_productcart" runat="server" />
         <%}%>       
        </p>
        ...
        <%}else {%>
            <asp:LinkButton ID="lnkbtn_ctaddtowaitlist" CssClass="lnkbtn_waitlist" runat="server" >Add to Wishlist</asp:LinkButton>
         <%}%> 
        ...
        <asp:Panel ID="pnl_userreg" runat="server" Visible="true">
            <div class="email_content">
                <div class="email_content_inner">
                    ...
                    <p><button id="btn_emailsubmit" runat="server">Submit</button></p>
                </div>
            </div>
        </asp:Panel>
    </td></tr>          
        ...
        <table width="100%"><tr><td>
          ...
          <asp:Panel ID="Panel1" runat="server" Visible="true">
            ...
            <div class="addlist_btn">
               <%-- <asp:LinkButton ID="lnkbtn_continue" CssClass="lnkbtn_cont" runat="server" >Continue Shopping</asp:LinkButton>--%>
            </div>
            </div>
           </div>
         </asp:Panel>
...
<div id="emptywaitlist_content">
    ...
    <div class="emptwaitlist_content">
        ...
        <div class="emptwaitlist_continue">
            ...
            <asp:LinkButton ID="lnkbtn_waitlist" CssClass="lnkbtn_cont" runat="server">Wish List</asp:LinkButton> in your account.</p>
            <span><asp:LinkButton ID="lnkbtn_continueshopping" CssClass="lnkbtn_cont" runat="server" >
