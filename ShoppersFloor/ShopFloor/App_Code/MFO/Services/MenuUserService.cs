...
namespace MFO.Services{
    ...
    public static class MenuUserService
    {
        ...
        {
            if (HttpContext.Current.Session[Constant.Session.PRODUCT_COUNT] == null)
            {
                int? productCount = menuDAO.getProductCount(HttpContext.Current.Session["UserID"] + "");
                HttpContext.Current.Session[Constant.Session.PRODUCT_COUNT] = productCount == null ? null : productCount;
                if (productCount != null)
                {
                    return productCount;
                }
            }
            else if (!StringUtil.isNullOrEmpty(HttpContext.Current.Session[Constant.Session.PRODUCT_COUNT]+""))
            {
                return (int?)HttpContext.Current.Session[Constant.Session.PRODUCT_COUNT];
            }
            return null;
